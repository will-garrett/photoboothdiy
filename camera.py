...
imagecounter = 0
imagefolder = 'Photos'
templatePath = os.path.join('Photos', 'Template', "template3.jpg") #Path of template image
ImageShowed = False
Printing = False
BUTTON_PIN = 25
#IMAGE_WIDTH = 558
...
def DisplayText(fontSize, textToDisplay):
    ...
    if (textToDisplay != ""):
            ...
            text = font.render(textToDisplay, 1, (225, 175, 76))
...
def UpdateDisplay():
    ...
    if (Message != ""):
            ...
            text = font.render(Message, 1, (225, 175, 76))
            textpos = text.get_rect()
            textpos.centerx = background.get_rect().centerx
            textpos.centery = background.get_rect().centery
            if(ImageShowed):
    ...
    if (Numeral != ""):
            ...
            text = font.render(Numeral, 1, (225, 175, 76))
            textpos = text.get_rect()
            textpos.centerx = background.get_rect().centerx
            textpos.centery = background.get_rect().centery
            if(ImageShowed):
    ...
    if (CountDownPhoto != ""):
            ...
            text = font.render(CountDownPhoto, 1, (225, 175, 76))
            textpos = text.get_rect()
            textpos.centerx = background.get_rect().centerx
            textpos.centery = background.get_rect().centery
            if(ImageShowed):
